...
export const trainRegimePawConfig: TrainRegimePawConfig = {
  ...
  phases: {
    observe: [
      'Run the retrainer locally, writing the new model to a staged path',
      'so the old joblib stays live while we verify it:',
      '',
      '  cd /Volumes/T7/Projects/trader-engine && uv run python scripts/train_regime.py --out models/regime_xgb.joblib.new; echo "EXIT_STATUS=$?"',
      '',
    ...
    analyze: [
      ...
      '  cat /Volumes/T7/Projects/trader-engine/models/regime_xgb.joblib.meta.json',
      '  for the',
    ...
    act: [
      ...
      'engine. The engine runs locally under launchd as of 2026-07-20;',
      'do NOT ssh to your-engine-host, that host is decommissioned',
      'and restarting it there would put two engines on one Alpaca',
      'account. Run:',
      '',
      '  cd /Volumes/T7/Projects/trader-engine && \\',
      '  DATE=$(date +%Y%m%d) && \\',
      '  mv models/regime_xgb.joblib models/regime_xgb_${DATE}.joblib.bak && \\',
      '  mv models/regime_xgb.joblib.new models/regime_xgb.joblib && \\',
      '  mv models/regime_xgb.joblib.meta.json models/regime_xgb_${DATE}.meta.json.bak && \\',
      '  mv models/regime_xgb.joblib.meta.json.new models/regime_xgb.joblib.meta.json && \\',
